...
-- Chapter 6: Functors, Applicative Functors, and Monads
...
def nisse : Helper where
  large := false
  assistance := "househouse tasks"
  payment := "bowl of porridge"
...
def hulde : SizedCreature where
  size := .medium

example : SizesMatch hulde := by simp
...
instance : Applicative Option where
  ...
  seq f x :=
    ...
    -- need to compute its argument, so x is never called
    | some g => g <$> x ()
...
-- Unfortunately, there is no Pair.pure
...
def checkYearIsNat (year : String) : Validate (Field × String) Nat :=
  ...
  | none => reportError "birthYear" "Must be digits"
  | some n => pure n
...
#eval checkInput 2020 {name := "John", birthYear := "1980"}
#eval checkInput 2020 {name := "", birthYear := "1980"}
#eval checkInput 2020 {name := "John", birthYear := "1980a"}
#eval checkInput 2020 {name := "John", birthYear := "1800"}
#eval checkInput 2020 {name := "John", birthYear := "2100"}

-- Form validation with checkInput illustrates a key advantage of Applicative
-- over Monad. Because >>= provides enough power to modify the rest of the
-- program's execution based on the value from the first step, it must receive a
-- value from the first step to pass on. If no value is received (e.g. because
-- an error has occurred), then >>= cannot execute the rest of the program.
-- Validate demonstrates why it can be useful to run the rest of the program
-- anyway: in cases where the earlier data isn't needed, running the rest of the
-- program can yield useful information (in this case, more validation errors).
...
-- E1 <|> E2 will use the built-in OrElse.orElse
...
def checkCompany (input : RawInput) : Validate (Field × String) LegacyCheckedInput :=
  pure (fun () name => .company name) <*>
    ...
    checkName input.name
-- this definition is quite noisy
...
#eval checkLegacyInput ⟨"Johnny", "1980"⟩
#eval checkLegacyInput ⟨"", "1963"⟩
#eval checkLegacyInput ⟨"", "1970"⟩

-- Alternative can help here
